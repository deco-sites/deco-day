...
export default function ScheduleMenu({
  ...
  return (
    <>
        ...
        <div id="agendaContainer" class={`absolute z-[999!important] ${agendaVisible.value ? 'transition-transform duration-700 ease-in-out transform translate-x-0 lg:translate-x-0' : 'transition-transform duration-500 ease-in-out transform translate-x-full hidden lg:translate-x-0'} h-full top-0 right-0 px-8 py-10 w-full lg:w-1/3 max-w-[350px] bg-white border border-gray-300 rounded p-4 shadow`}>            <span
            onClick={toggleAgenda}
            ...
            <div class="flex flex-col gap-y-8">
                <div class="flex flex-col items-start gap-y-8">
                    <span class="text-black text-[24px] leading-tight tracking-tight">{infoPanel.scheduleHeader?.date}</span>
                    <div class="flex flex-col gap-2 max-w-[231px]">
                        <span class="text-black text-[24px] leading-tight tracking-tight">{infoPanel.scheduleHeader?.loc}</span>
                        <span class="text-black font-albert-sans text-base leading-tight tracking-tight">{infoPanel.scheduleHeader?.fullLoc}</span>
                    </div>
                </div>
                <div class="flex flex-col">
                    <h3 class="text-albert-sans text-base leading-tight tracking-tight py-6 border-b border-black border-solid border-opacity-50">Agenda</h3>
